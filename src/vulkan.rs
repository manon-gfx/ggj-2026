...
pub(crate) struct VulkanState {
    ...
    upload_texture_allocations: Vec<(vk::DeviceMemory, *mut std::ffi::c_void)>,
    upload_textures: Vec<vk::Image>,

    command_buffers: Vec<vk::CommandBuffer>,
    _command_pool: vk::CommandPool, // RAII
    ...
    _entry: Entry, // Must keep-alive or the vulkan lib gets unloaded haha
...
impl Drop for VulkanState {
    fn drop(&mut self) {
        // Sync the GPU
        unsafe { self.device.wait_for_fences(&self.fences, true, 0).unwrap() };

        // Cleaning up is a waste of time if we are exiting the application
        // I don't forsee runtime backend switches, so let's just assume we can leak and let the OS
        // deal with clean-up.
    }
}

impl VulkanState {
    pub(crate) fn acquire_bitmap(&mut self) -> Bitmap {
        assert!(
            self.image_index.is_none(),
            "`acquire_bitmap` called again before calling 'blit_to_screen`"
        );

        // Wait for any previous work on the GPU to finish
        let fence = self.fences[self.swap_index];
        unsafe {
            self.device
        ...
        let image_acquired_semaphore = self.image_acquired_semaphores[self.swap_index];
        ...
        self.image_index = Some(image_index as usize);

        // Set up a bitmap to write to the upload buffer
        let mapped_ptr = self.upload_texture_allocations[self.swap_index].1;
        Bitmap::new_borrowed(
            mapped_ptr as *mut _,
            self.render_width,
            self.render_height,
    ...
    pub(crate) fn blit_to_screen(
        &mut self,
        _bitmap: Bitmap, /* we already got the pointer to the data */
    ) {
        let image_index = self
            .image_index
            .take()
            .expect("`blit_to_screen` called without calling `acquire_bitmap` first.");

        let fence = self.fences[self.swap_index];
        let cmd = self.command_buffers[self.swap_index];
        ...
        unsafe {
            ...
            let upload_texture = self.upload_textures[self.swap_index];
            ...
            let scale_x = self.window_width as f32 / self.render_width as f32;
            let scale_y = self.window_height as f32 / self.render_height as f32;
            let scale = scale_x.min(scale_y);

            let blit_width = (self.render_width as f32 * scale) as i32;
            let blit_height = (self.render_height as f32 * scale) as i32;

            let blit_x = (self.window_width as i32 - blit_width) / 2;
            let blit_y = (self.window_height as i32 - blit_height) / 2;
            ...
            self.device.cmd_blit_image(
                ...
                &[vk::ImageBlit::default()
                    ...
                    .src_offsets([
                        vk::Offset3D::default(),
                        vk::Offset3D {
                            x: self.render_width as i32,
                            y: self.render_height as i32,
                            z: 1,
                        },
                    ])
            ...
            let rendering_complete_semaphore = self.rendering_complete_semaphores[self.swap_index];
            // Submit command buffer to the GPU Queue
            {
                let wait_semaphores = [self.image_acquired_semaphores[self.swap_index]]; // start rendering when swapchain image is free
            ...
            self.swap_index = (self.swap_index + 1) % IN_FLIGHT_COUNT as usize;
        };
    }
}
...
pub(crate) fn init_vulkan(
    ...
    let entry = match unsafe { Entry::load() } {
        Ok(entry) => entry,
        Err(err) => {
            println!("Failed to load vulkan =[. \"{}\"", err);
            return None;
        }
    };
    ...
    let vk_instance = match unsafe { entry.create_instance(&create_info, None) } {
        Ok(instance) => instance,
        Err(err) => {
            println!("Failed to create a vulkan instance =[. \"{}\"", err);
            return None;
        }
    };
    ...
    if physical_devices.is_empty() {
        println!("Failed to find a vulkan device :(");
        return None;
    }
    ...
    Some(VulkanState {
        ...
        render_width,
        render_height,
        render_stride: render_stride as usize,
    })
}
